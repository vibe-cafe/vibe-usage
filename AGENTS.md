...
- **Local privacy controls** — `uploadProject=false` is an authoritative local deny and skips the settings request; a server `false` still wins when the local value is `true` or absent. `uploadHostname=false` replaces local hostnames in buckets, sessions, and client metadata with a persistent `device-<hex>` id at the final pre-hash/network boundary. The non-identifying `cursor-cloud` sentinel remains shared for cross-device dedup. Never let a parser bypass this layer. New setups default both controls off; configs without the keys keep legacy behavior. Toggling hostname privacy migrates unchanged local bucket-state keys so it does not duplicate all history; removing identifiers already stored remotely remains an explicit full `reset`.
- **Hidden-project identity** — parsers aggregate before privacy is applied. When effective project upload is false, `sync.js` replaces project names with `unknown` and must re-aggregate buckets before hashing/upload so formerly distinct projects that now share a server key are summed instead of overwriting one another.
...
- **Stable device identity** — when device-name upload is enabled, `hostname` is persisted at init/first sync and never re-read after capture, preventing macOS mDNS drift (`-2`, `-3`) from duplicating devices. When disabled, the generated `deviceId` is the stable identity and `os.hostname()` is not read at all.
