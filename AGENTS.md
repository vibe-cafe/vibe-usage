...
- Replay dedup is verified by content, never by `session/end-seed` marker position: DSH (dev preview) writes that marker at creation, at resume boundaries, and appends one at the END of a file when the session becomes a seed for a further resume, so "skip before the last marker" discards real history. Only files whose header carries `parentSession` are treated as fork/resume files: their leading `user/message` + `assistant/message` records are fingerprinted (sha256 of the record minus `seq`/`time`), and only the longest leading prefix that is also a suffix of the parent's message stream at the child's `createdAt` is skipped (KMP, same idea as the Codex replay matcher; chunk/bookkeeping records are excluded because they are re-serialized in replays and would break the exact match). Matches below `MIN_REPLAY_MESSAGES` (3) fail open and count everything, as do files whose parent is missing — the bias is to over-count rather than drop a session's own first turns. Files without a `parentSession` are counted in full: their `session/end-seed` markers are resume/seed bookkeeping, not replay boundaries. A session id appearing in several project dirs keeps the most complete copy.
