...
- Session logs at `$DSH_HOME/sessions/<project-key>/session-<id>/session.jsonl.zstd` (default `~/.dsh`; fixture/relocation override `VIBE_USAGE_DSH_SESSIONS`) are multi-frame Zstandard — one frame per write batch. Node's `node:zlib` zstd (Node ≥ 22.15) decodes exactly one frame per call, so `splitZstdFrames()` walks the RFC 8878 frame structure (magic/header/blocks/checksum, skippable frames) and decompresses each frame separately; a torn non-frame tail is ignored, mirroring DSH's own truncation repair. On older Node the `zstd` CLI (`zstd -d -c`) is the fallback; if neither exists the parser returns `skipped` with a hint so prior state is not pruned. Plain `session.jsonl` logs (compression disabled) are read directly.
- Usage comes from `assistant/message` records: `usage.inputTokens` (uncached input) → `inputTokens`, `usage.cacheReadTokens` → `cachedInputTokens`; `usage.outputTokens` includes `reasoningTokens` (verified against DSH's own session_projcache totals), so reasoning is subtracted from output into `reasoningOutputTokens`. Per-message usage carries no cache-write field.
- Everything before the LAST `session/end-seed` marker is a replay of a resumed/forked session's seed history that was already counted from its original file — skip it or usage doubles. A session id appearing in several project dirs keeps the most complete copy.
