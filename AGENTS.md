...
- A sub-agent rollout (`session_meta.payload.thread_source === 'subagent'`, a `source: { subagent: ... }` object, or a `parent_thread_id` — the marker varies by Codex version, so all three are checked) can begin with the parent conversation's history copied in, `token_count` records included, all re-stamped at the spawn instant. Counting those again on top of the parent's own file inflated one user's 16:00–17:00 hour by ~22× (974M reported vs ~44M real). There is no `forked_from_id`/count to size the copied block by; instead it ends at the sub-agent's **own first `task_started`** event, so the parser skips every record before it (token counts *and* session timing events; `session_meta` is kept as the session-start marker). A sub-agent file with no `task_started` at all skips nothing — same over-count-beats-drop policy as missing fork sources.
