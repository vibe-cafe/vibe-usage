...
| Tool | Data Location |
|------|---------------|
| Alma | Electron app-data `alma/chat_threads.db` (macOS: `~/Library/Application Support/alma/chat_threads.db`; fixture override: `VIBE_USAGE_ALMA_DB`). Reads only `usage_records` plus thread/workspace IDs and workspace names; it never reads message bodies, metadata, provider credentials, or full workspace paths. Cache-write tokens are included in input usage. Alma usage rows represent assistant responses only, so this integration emits 30-minute token buckets but not sessions rather than reading message content to infer user/assistant timing. Windows/Linux paths follow the standard Electron app-data convention but are conservatively unverified. The parser emits source `alma`; the private backend source registry must be updated by the maintainer before Alma buckets are accepted. |
...
- Extracts session metadata where the source safely exposes user/assistant timing: active time (AI generation time, excluding queue/TTFT wait), total duration, message counts. Alma intentionally emits buckets only because its usage rows represent assistant responses and the parser does not read message bodies or metadata.
...
- SQLite-backed tools (Alma, Cursor, OpenCode, Kiro, Hermes) are read via Node's built-in `node:sqlite` on Node ≥ 22.5 — no `sqlite3` binary needed (works on Windows out of the box); on older Node it falls back to the system `sqlite3` CLI
