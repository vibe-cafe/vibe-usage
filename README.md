...
| DeepSeek Harness | `$DSH_HOME/sessions/` (default `~/.dsh`, fixture/relocation override: `VIBE_USAGE_DSH_SESSIONS`). Reads multi-frame Zstandard `session.jsonl.zstd` logs (built-in `node:zlib` zstd on Node ≥ 22.15, `zstd` CLI fallback); usage from `assistant/message` records (uncached input, cache reads, reasoning split out of output); the seed replay before the last `session/end-seed` marker is skipped so resumed/forked history is not double-counted |
